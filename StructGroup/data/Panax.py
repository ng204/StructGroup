...
class PanaxDataset(CustomDataset):

    CLASSES = ('stem', 'leaf', 'branch')  # 3个类别（即使某些数据中没有branch）
    ...
    def load(self, filename):
        # Panax 预处理 .pth 已经包含：
        #   semantic_label: 0(stem), 1(leaf), 2(branch)
        #   instance_label: 0,1,2,... (同一实例同一 id，-100 为 ignore)
        # 训练阶段需要的是「连续的实例 id」，用于计算 GT center / offset：
        #   - 在 CustomDataset.getInstanceInfo 里直接按实例 id 聚合中心
        #   - 在 collate_fn 里通过 +total_inst_num 做跨场景偏移
        #
        # 之前这里尝试将 instance_label 转成 sem*1000+inst 的编码形式，
        # 会破坏这一假设，导致：
        #   - instance_label.max() 变成几千，遍历大量不存在的 inst_id
        #   - collate_fn 再次对「已编码的 label」加偏移，语义 / 实例位全部错乱
        #   - 最终 GT center / offset 全部错误，网络学到“把 leaf 推到错误中心”
        #
        # 因此，这里保持原始的连续 instance_label 即可，不再做任何重编码。
        xyz, rgb, semantic_label, instance_label, _, _ = torch.load(filename)
        
        # 如果场景过大，随机采样避免crop失败
        if self.training and xyz.shape[0] > self.voxel_cfg.max_npoint:
            inds = np.random.choice(xyz.shape[0], self.voxel_cfg.max_npoint, replace=False)
            xyz = xyz[inds]
            rgb = rgb[inds]
            semantic_label = semantic_label[inds]
            instance_label = instance_label[inds]
        
        # subsample data for x4_split
